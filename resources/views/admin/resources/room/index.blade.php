...
<div class="app-content-header">
    <div class="container-fluid">
        <div class="row">
            <div class="col-sm-6"><h3 class="mb-0">Data Ruangan</h3></div>
            <div class="col-sm-6">
...
<div class="app-content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-header"><h3 class="card-title">Ruangan</h3></div>
                    <div class="col-12 d-flex">
                        @if(auth()->user()->hasRole(['admin', 'supervisor', 'petugas']))
                        <a href="{{ url('configuration/rooms/create') }}" class="btn btn-primary ms-3 mt-3">
                            Tambah
                        </a>
                        <a href="{{ url('configuration/rooms/invoice') }}" class="btn btn-warning ms-3 mt-3">
                            Invoice
                        </a>
                        @endif
                    </div>
                    <div class="card-body">
                        <table id="dataTableRoom" class="table table-striped" style="width:100%">
                            <thead>
                                <tr>
                                    <th>No</th>
                                    <th>Gedung</th>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{ $r->building ? $r->building->buildingName : 'building not set'  }}</td>
                                    <td>{{ $r->roomName  }}</td>
                                    <td>{{ $r->floor  }}</td>
                                    <td>
                                        @if(auth()->user()->hasRole(['admin', 'supervisor', 'petugas']))
                                        <a href="{{ url('/configuration/rooms/edit/' . $r->roomId) }}" class="btn btn-primary">Edit</a>     
                                        @endif
                                        @if(auth()->user()->hasRole(['admin']))                              
                                        <form action="{{ url('configuration/rooms/delete', $r->roomId) }}" method="POST" style="display: inline;">
                                            @csrf
                                            @method('POST') 
